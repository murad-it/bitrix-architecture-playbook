...
## 13) Что проверить перед merge в main
...
D7 ORM в 1C-Битрикс хорошо работает там, где команда не прячет SQL-природу запроса за «красивым PHP».  
Когда запросы проектируются осознанно, JOIN проверяется по плану, а изменения проходят через метрики, ORM действительно ускоряет разработку и делает систему стабильнее в поддержке.
